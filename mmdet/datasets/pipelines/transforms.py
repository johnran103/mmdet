...
@PIPELINES.register_module()
class ResizeWithDensityMap:
    """Resize images & bbox & mask & density map.
    ...
    def _resize_density_map(self, results):
        """Resize density map with ``results['scale']``."""
        for key in results.get('density_map_fields', []):
            if self.keep_ratio:
                density_map = mmcv.imrescale(
                    results[key],
                    results['scale'],
                    interpolation='nearest',
                    backend=self.backend)
            else:
                density_map = mmcv.imresize(
                    results[key],
                    results['scale'],
                    interpolation='nearest',
                    backend=self.backend)

            results[key] = density_map
    ...
    def __call__(self, results):
        ...
        self._resize_masks(results)
        self._resize_seg(results)
        self._resize_density_map(results)
        return results
...
@PIPELINES.register_module()
class ResizeWithQualityMapAndScaleMap:
    """Resize images & bbox & mask & quality map & scale.
    ...
    def _resize_quality_map(self, results):
        """Resize density map with ``results['scale']``."""
        for key in results.get('quality_map_fields', []):
            if self.keep_ratio:
                quality_map = mmcv.imrescale(
                    results[key],
                    results['scale'],
                    interpolation='nearest',
                    backend=self.backend)
            else:
                quality_map = mmcv.imresize(
                    results[key],
                    results['scale'],
                    interpolation='nearest',
                    backend=self.backend)

            results[key] = quality_map

    def _resize_scale_map(self, results):
        """Resize density map with ``results['scale']``."""
        for key in results.get('scale_map_fields', []):
            if self.keep_ratio:
                scale_map = mmcv.imrescale(
                    results[key],
                    results['scale'],
                    interpolation='nearest',
                    backend=self.backend)
            else:
                scale_map = mmcv.imresize(
                    results[key],
                    results['scale'],
                    interpolation='nearest',
                    backend=self.backend)

            results[key] = scale_map
    ...
    def __call__(self, results):
        ...
        self._resize_masks(results)
        self._resize_seg(results)
        self._resize_quality_map(results)
        self._resize_scale_map(results)
        return results
...
@PIPELINES.register_module()
class RandomFlipWithDensityMap:
    """Flip the image & bbox & mask.
...
@PIPELINES.register_module()
class RandomFlipWithQualityMapAndScaleMap:
    """Flip the image & bbox & mask.
...
class PadWithDensityMap:
    ...
    def __init__(self,
                 ...
                 size_divisor=None,
                 pad_to_square=False,
                 pad_val=dict(img=0, masks=0, seg=255, density_map=0)):
        self.size = size
        self.size_divisor = size_divisor
        if isinstance(pad_val, float) or isinstance(pad_val, int):
            warnings.warn(
    ...
    def _pad_deisity_map(self, results):
        """Pad deisity map according to
        ``results['pad_shape']``."""
        pad_val = self.pad_val.get('density_map', 0)
        for key in results.get('density_map_fields', []):
            results[key] = mmcv.impad(
                results[key], shape=results['pad_shape'][:2], pad_val=pad_val)
    ...
    def __call__(self, results):
        ...
        self._pad_masks(results)
        self._pad_seg(results)
        self._pad_deisity_map(results)
        #result_img_type = type(results['img'])
        #result_density_map_type = type(results['density_map'])
        #print(f'result_img_type {result_img_type}')
        #print(f'result_density_map_type {result_density_map_type}')
        #print(results['img'].shape)
        #print(results['density_map'].shape)
        return results
...
@PIPELINES.register_module()
class PadWithQualityMapAndScaleMap:
    """Pad the image & masks & segmentation map.
    ...
    def _pad_quality_map(self, results):
        """Pad quality map according to
        ``results['pad_shape']``."""
        pad_val = self.pad_val.get('quality_map', 0)
        for key in results.get('quality_map_fields', []):
            results[key] = mmcv.impad(
                results[key], shape=results['pad_shape'][:2], pad_val=pad_val)
    
    def _pad_scale_map(self, results):
        """Pad scale map according to
        ``results['pad_shape']``."""
        pad_val = self.pad_val.get('scale_map', 0)
        for key in results.get('scale_map_fields', []):
            results[key] = mmcv.impad(
                results[key], shape=results['pad_shape'][:2], pad_val=pad_val)
    ...
    def __call__(self, results):
        ...
        self._pad_masks(results)
        self._pad_seg(results)
        self._pad_quality_map(results)
        self._pad_scale_map(results)
        #result_img_type = type(results['img'])
        #result_density_map_type = type(results['density_map'])
        #print(f'result_img_type {result_img_type}')
        #print(f'result_density_map_type {result_density_map_type}')
...
class AlbuAdditional:
    ...
    Args:
        ...
        skip_img_without_anno (bool): Whether to skip the image if no ann left
            after aug
        additional_targets: additional targets using the same set of aug
    """
    ...
    def __init__(self,
        ...
        if additional_targets is not None:
            additional_targets = copy.deepcopy(additional_targets)
        self.transforms = transforms
        self.filter_lost_elements = False
        self.update_pad_shape = update_pad_shape
        self.skip_img_without_anno = skip_img_without_anno
