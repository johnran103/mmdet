...
class DYMPHead3(GFLHead):
    ...
    def __init__(self,
        ...
        super(DYMPHead3, self).__init__(**kwargs)

        self.atten1 = attention1d(in_planes=2, ratios=16, K=4)
        self.atten2 = attention1d(in_planes=2, ratios=16, K=4)
        in_channel = 256 * 5
        self.c_attens = nn.ModuleList()
        self.s_attens = nn.ModuleList()
        self.bns = nn.ModuleList()

        self.scale_s_atten = SpatialAttention()
        self.scale_s_atten = SpatialAttention()
    ...
    def dynamic_net(self, feat, weight):
    ...
    def refine_feat_by_scale(self, feats, scale_map=None):
        ...
        s_feat = torch.cat(_feats, dim=1)
        s_feat = self.scale_s_atten(s_feat)

        scale_map = torch.cat([s_feat,scale_map],dim=1)
        ...
        for i in range(len(_feats[0])):
            ...
            _out = self.dynamic_net(_input, weight[i]) # split here
    ...
    def refine_feat_by_quality(self, feats, quality_map=None):
        ...
        sz = [x.size() for x in feats]
        mid_sz = sz[0][-2:]
        quality_map = F.interpolate(quality_map, size=mid_sz)
        ...
        quality_map = torch.cat([s_feat,quality_map],dim=1)

        weight = self.atten2(quality_map)
        for i in range(len(_feats[0])):
            _input = []
            for j in range(num_scale):
                _input.append(_feats[j][i])
