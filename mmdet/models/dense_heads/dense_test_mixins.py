...
class BBoxTestMixin(object):
    ...
    def simple_test_bboxes(self, feats, img_metas, rescale=False):
        ...
        #outs,quality_map = self.forward(feats)
        #print(f'scale_maps[2] {scale_maps[2].size()}')
        #print(f'img_metas {img_metas}')

        #save_path = img_metas[0]['filename'].split('/')[-1]
        #save_path = save_path.replace('jpg','npy')

        #save_path = '/home/ranqinglin/work_dirs/ddet/work_dirs/dy_mp_det_res50_sw/check_quality/epoch_12/' + save_path
        #quality_map = np.array(quality_map[0][0].cpu())

        #print(f'quality_map shape {quality_map.shape}')
        #np.save(save_path, quality_map)
    ...
    def aug_test_bboxes(self, feats, img_metas, rescale=False):
        ...
        for x, img_meta in zip(feats, img_metas):
            ...
            outs = self.forward(x)
            # #outs = self.forward(feats, train=True)

            # density_map = outs[-1]

            # print(f'len of density_map {len(density_map)}')

            # density_map = density_map[0].cpu().numpy()
    
            # np.save('./test_density_map.npy', density_map)
