...
class DYMPHeadSW(GFLHead):
    ...
    def __init__(self,
        ...
        super(DYMPHeadSW, self).__init__(**kwargs)
    ...
    def _init_layers(self):
        ...
        self.dy_level1 = Dynamic_conv1d(self.feat_channels, 4) # here 4 means dynamic network C2
        ...
        self.dy_level_all = Dynamic_conv1d(self.feat_channels, 4)
        ...
        self.quality_mapper = nn.Conv2d(self.feat_channels, 1, 1)
    ...
    def forward_single(self, x, scale, quality_map=None, top_size=None):
        ...
        x = self.dy_level_all(x, new_quality_map.detach().clone())
        x = self.BN1(x)
        x = self.relu(x)
    ...
    def loss_quality_all_level(self, quality_map, gt_quality_map):
        ...
        np.save('/home/ranqinglin/work_dirs/ddet/visualize_quality/quality_map.npy',np.array(quality_map))
        np.save('/home/ranqinglin/work_dirs/ddet/visualize_quality/gt_quality_map.npy',np.array(gt_quality_map))
        exit(0)
    ...
    def predict_quality(self, feats):
        '''
            ...
            quality_map: Tensor
        '''
        sz = feats[0].size()[-2:] # low-level feature map
        num_scale = len(feats)
        _N = len(feats[0])
        indv_feat = [[] for _ in range(_N)]
        for i in range(num_scale):
            for j in range(_N):
                indv_feat[j].append(F.interpolate(feats[i][j][None, :, :, :], size=sz)) # (1, 256, H, W)
        ...
        #indv_feat = self.relu(indv_feat) wrong order.
    ...
    def get_new_feat(self, feats, training=None):
        ...
        for i in range(_N):
            indv_feat[i] = torch.cat(indv_feat[i], dim=1) # (1, 256 * 5, H, W)
        
        indv_feat = torch.cat(indv_feat) #(N, 256 * 5, H, W)
        ...
        for i in range(self.fuse_stage):
            ...
            scale_map = self.scale_mappers[i](scale_map) # (2, 256, H, W)
            indv_feat = self.fuse_info_via_dyconv(i, indv_feat, scale_map.detach().clone())
            indv_feat = self.BN1(indv_feat)
            indv_feat = self.relu(indv_feat)
    ...
    def forward(self, feats, training=False):
        ...
        if training:
            ...
            return ret, quality_map # modified
