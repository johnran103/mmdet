...
class DYMPHead1GT_Q(GFLHead):
    ...
    def __init__(self,
        ...
        super(DYMPHead1GT_Q, self).__init__(**kwargs)
    ...
    def refine_feat_by_scale(self, feats, scale_map=None):
        ...
        if len(scale_map.size()) == 3:
            scale_map = scale_map[None,...]
    ...
    def refine_feat_by_quality(self, feats, quality_map=None):
        ...
        sz = [x.size() for x in feats]
        mid_sz = sz[1][-2:]
        #print(quality_map.size())
        if len(quality_map.size()) == 3:
            quality_map = quality_map[None,...]
        ...
        shape = [256 for i in range(len(feats))]
        weight = self.atten1(quality_map)
        for i in range(len(_feats[0])):
            _input = []
            for j in range(num_scale):
                _input.append(_feats[j][i])
            _input = torch.cat(_input)[None,...]
            _out = self.dynamic_net_scale(_input, quality_map, weight[i]) # split here
            _out = torch.split(_out, shape, dim=1)
            all_feats.append(_out)
