...
class MPHeadT(GFLHead):
    ...
    def __init__(self,
        ...
        self.beta = beta
        self.gamma = gamma
        super(MPHeadT, self).__init__(**kwargs)
        
        self.nums = [79337, 27059, 29647, 10480, 12875, 144866, 24956, 5926, 4812, 3246]
        self.exp = exp
        self.weight = []
        self.s = 0
        for i in range(len(self.nums)):
            self.s += (1.0 / self.nums[i]) ** self.exp
        self.half_s = self.s / 3.0
        for i in range(len(self.nums)):
            self.weight.append((self.half_s + (1.0 / self.nums[i]) ** self.exp) / self.s)
    ...
    def _get_bboxes_single(self,
        ...
        for level_idx, (cls_score, bbox_pred, stride, priors) in enumerate(
            ...
            #print(scores.size())
            scores = scores * self.weight[None, None, ...].to(scores.device).squeeze(0)
            #print(scores.size())
