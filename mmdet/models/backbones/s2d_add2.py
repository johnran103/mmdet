...
class Conv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        ...
        # ------ batchnormalization ---------------
        _, self.bn = build_norm_layer(dict(type='BN', requires_grad=True), c2) # modified from SyncBN -> BN
...
class Space2DepthChainAdd2(nn.Module):
    def __init__(self, in_channels=16):
        super(Space2DepthChainAdd2, self).__init__()
        channels = [128, 128, 128, 256, 512, 1024, 2048] # wen can tune anywhy
        ...
        self.focus_256x = Focus(c1=channels[4]*4, c2=channels[5], b=2)
        self.focus_512x = Focus(c1=channels[5]*4, c2=channels[6], b=2)
    ...
    def forward(self, x):
        ...
        outs.append(x_128x)
        x_256x = self.focus_256x(x_128x)
        outs.append(x_256x)
        x_512x = self.focus_512x(x_256x)
        outs.append(x_512x)
