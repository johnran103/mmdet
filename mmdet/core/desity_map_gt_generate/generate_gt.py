...
def gaussian_filter_density(non_zero_points, map_h, map_w, distances=None, kernels_dict=None, min_sigma=2, method=1,
    ...
    for i in range(gt_count):
        ...
        sigma = annotation_stats[category]
...
if __name__ == "__main__":
    ...
    img_paths = glob.glob(f'{root_dir}/images/instances_UAVtrain/*.jpg')
    method = 3
    const_sigma = 15
    ...
    with open(str(mode) + ".txt", "w") as fileloader:
        ...
        for img_path in tqdm(img_paths):
            img_path = '/home/ranqinglin/work_dirs/ddet/data/uav-dataset/VisDrone/images/instances_UAVtrain/0000007_04999_d_0000036.jpg'
            fileloader.write(img_path)
            fileloader.write("\n")
            data_folder, img_sub_path = img_path.split('images')

            #ann_path = img_path.replace(img_affix, '.txt')
            #ann_path = ann_path.replace("images", 'raw_annotations')

            img_name = img_path.split('/')[-1]
            ann_name = img_name.replace('.jpg', '.txt')
            #print(root_dir)
            ann_path = os.path.join('/home/ranqinglin/work_dirs/ddet/data/uav-dataset/VisDrone/raw_annotations', ann_name)
            #print(ann_path)
            ...
            # import imageio
            # print(img_path)
            # imageio.imwrite('./density_map.jpg', density_map*1000)
            # exit(0)
