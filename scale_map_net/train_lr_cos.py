...
from s_net import MCNN, res50_fpn
...
if __name__=="__main__":
    ...
    dataset = ScaleDataset(img_root,gt_dmap_root,4)
    ...
    #training phase
    if not os.path.exists('./checkpoints_cos'):
        os.mkdir('./checkpoints_cos')
    ...
    for epoch in range(0, max_epoch):
        ...
        for i,(img,gt_dmap) in enumerate(dataloader):
            ...
            # w1 = min(math.cos((max_epoch-(epoch-10))/max_epoch*(pi/2)), 0.5)
            # w2 = max(math.cos((epoch-10)/max_epoch*(pi/2)), 0.5)
        ...
        if (epoch + 1) % 5 == 0:
            torch.save(mcnn.state_dict(),'./checkpoints_cos/epoch_'+str(epoch)+".pth")
